...
export async function POST(request: Request) {
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      to: userEmail,
      subject: `Alerta de stock critico: ${productName}`,
      html: `
        <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; padding: 2rem;">
          <h2 style="color: #a8e063;">Alerta de stock critico</h2>
          <p>El producto <strong>${productName}</strong> ha alcanzado nivel critico de inventario.</p>
          <div style="background: #1a1f16; border: 1px solid #2a3024; border-radius: 8px; padding: 1rem; margin: 1.5rem 0;">
            ...
            <p style="color: #7a8a6a; margin: 0;">Minimo recomendado: ${minStock} unidades</p>
          </div>
          ...
          <p style="color: #7a8a6a; font-size: 0.8rem; margin-top: 2rem;">NotiStock</p>
        </div>
      `,
    });
